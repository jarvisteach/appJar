...
* `.startSubwindow(name, title=None, modal=False, transient=False, blocking=False, grouped=True)` & `.stopSubwindow()`  
    ...
    Setting `grouped` to False, will remove this *SubWindow* from the window group, stopping it from hiding, when the main window is hidden.  
